...
import {resolveColor, nodeXPositions, layoutPositions, boxSize, THEMES, LAYOUTS, COLORS} from './spec.ts';
...
test('nodeXPositions keeps 4 nodes on-canvas (width-aware half)', () => {
  const half = boxSize('nodes-flow', 4).w / 2;
  const xs = nodeXPositions(4);
  assert.equal(xs.length, 4);
  for (const x of xs) {
    assert.ok(Math.abs(x) + half <= 540, `x=${x} pushes box off the 1080-wide canvas`);
  }
});
