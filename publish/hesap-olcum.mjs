...
export const HESAP_METRIKLERI = [
  {ad: 'reach', params: 'period=day'},
  {ad: 'profile_views', params: 'period=day'},
  {ad: 'follows_and_unfollows', params: 'period=day&metric_type=total_value'},
];
...
export async function hesapMetrikleri({igUserId, token, fetchFn = fetch, metrikler = HESAP_METRIKLERI}) {
  ...
  for (const m of metrikler) {
    ...
    try {
      ...
      // period=day → values[]; metric_type=total_value → total_value.value
      const deger = kalem?.total_value?.value ?? kalem?.values?.at(-1)?.value ?? null;
      if (deger === null) throw new Error('değer yok (metrik adı değişmiş olabilir)');
      out[m.ad] = deger;
    } catch (err) {
