...
//
// Saf fonksiyon: script gövdesinden çıkarıldı ki test import ederken CLI kısmı çalışmasın
// (bkz. aşağıdaki process.argv[1] muhafazası) ve --yasak mantığı testlerle doğrulanabilsin.
// TR-aware: 'İ'.toLowerCase() === 'i̇' (nokta kalır, "ipek" ile eşleşmez) — toLocaleLowerCase('tr')
// hem 'İ'→'i' hem 'I'→'ı' dönüşümünü doğru yapar (bkz. gözden-gecirici bulgusu 2026-08-26).
export function yasakTara(spec, liste) {
  if (!liste?.length) return [];
  const alanlar = {title: spec?.title ?? '', hook: spec?.hook ?? '', subject: spec?.subject ?? ''};
  const isabetler = [];
  for (const [alan, metin] of Object.entries(alanlar)) {
    const alt = String(metin).toLocaleLowerCase('tr');
    for (const kelime of liste) if (alt.includes(kelime)) isabetler.push({alan, metin, kelime});
  }
  return isabetler;
}

// CLI: yalnız doğrudan çalıştırılınca (node brain/dene-spec.mjs …) — import edilince (test)
// arg parse / process.exit / gerçek Gemini çağrısı TETİKLENMEZ.
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  const brandArg = arg('brand');
  const pillarKey = arg('pillar');
  const twistKey = arg('twist');
  const yasakListesi = (arg('yasak') ?? '').split(',').map(s => s.trim().toLocaleLowerCase('tr')).filter(Boolean);
  ...
  if (yasakListesi.length) {
    const yasakIsabet = yasakTara(spec, yasakListesi).map(({alan, metin, kelime}) => `${alan}: "${metin}" ← "${kelime}"`);
